...
fn main() {
    ...
    for i in 0..20 {
        let handle = std::thread::spawn(move || {hello_thread(i); });  // closures are just inline functions, like when we do error.Go(func(){}), because in spawn, it takes a function that takes no argument, so we use move to take ownership of i
