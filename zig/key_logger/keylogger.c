// This tells the compiler we're targeting Windows 2000 or later, for compatibility with WH_KEYBOARD_LL
...
LRESULT CALLBACK KeyboardProc(int nCode, WPARAM wParam, LPARAM lParam) {
    ...
    // Checks whether the key event is a key press. WM_SYSKEYDOWN is used for keys like ALT.
  if (nCode == HC_ACTION && (wParam == WM_KEYDOWN || wParam == WM_SYSKEYDOWN)) {

    /*
    Casts the lParam to a pointer to a KBDLLHOOKSTRUCT struct to access details about the key event.
    vkCode is the virtual key code of the key that was pressed (e.g., 65 for 'A').
    */
...
0: thread ID (0 = hook all threads system-wide).
  */
  ...
  /*
  GetMessage(&msg, NULL, 0, 0): Retrieves the next message from the message queue.

TranslateMessage(&msg): Processes the message for character translation.

DispatchMessage(&msg): Sends the message to the window procedure or callback function (KeyboardProc) for further processing.
  */
